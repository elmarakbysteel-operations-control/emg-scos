...
import { Plus, Edit, Trash2, FileSpreadsheet, Download } from "lucide-react";
...
export default function Procurement() {
  ...
  const handleExcelExport = () => {
    exportToExcel(items || [], [
      { header: "PR No", key: "pr", getValue: (r: any) => r.prNumber || "—" },
      { header: "PO No", key: "po", getValue: (r: any) => r.poNumber || "—" },
      { header: "Supplier", key: "supplier", getValue: (r: any) => r.supplierName || "—" },
      { header: "Material", key: "material", getValue: (r: any) => r.material || "—" },
      { header: "Quantity", key: "qty", getValue: (r: any) => r.quantity || 0 },
      { header: "Unit Price", key: "unitPrice", getValue: (r: any) => r.unitPrice || 0 },
      { header: "Total Value (USD)", key: "value", getValue: (r: any) => r.totalValue || 0 },
      { header: "Status", key: "status", getValue: (r: any) => r.status || "—" },
      { header: "Approval", key: "approval", getValue: (r: any) => r.approvalStatus || "—" },
    ], `Procurement_${new Date().toISOString().slice(0, 10)}.xlsx`, "Procurement");
    toast.success("Exported to Excel");
  };
  const handlePdfExport = () => {
    exportToPdf(items || [], [
      { header: "PR No", getValue: (r: any) => r.prNumber || "—" },
      { header: "PO No", getValue: (r: any) => r.poNumber || "—" },
      { header: "Supplier", getValue: (r: any) => r.supplierName || "—" },
      { header: "Material", getValue: (r: any) => r.material || "—" },
      { header: "Total Value (USD)", getValue: (r: any) => r.totalValue || 0 },
      { header: "Status", getValue: (r: any) => r.status || "—" },
    ], "Procurement Report", `Procurement_${new Date().toISOString().slice(0, 10)}.pdf`);
